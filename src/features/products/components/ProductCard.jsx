...
import { formatPrice, getDiscount } from '../../../shared/utils/helpers'
...
export default function ProductCard({ product }) {
  ...
  const handleAddToCart = (e) => {
    e.preventDefault()
    if (product.stock === 0) {
      feedback.notifyWarning('This item is currently out of stock')
      return
    }
    addToCart(product)
    setIsDrawerOpen(true)
    feedback.cartAdded(product)
  }
      ...
      <div className="relative aspect-[4/5] overflow-hidden bg-navy-950">
        <Link to={`/product/${product.id}`} className="block h-full">
          <img
            ...
            className={`absolute inset-0 w-full h-full object-cover transition-all duration-1000 ease-out-expo ${isHovered && product.secondaryImage && !secondaryImgError ? 'opacity-0 scale-110 blur-sm' : 'opacity-100 scale-100'}`}
          />
          {product.secondaryImage && !secondaryImgError && (
            <img
